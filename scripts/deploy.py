...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'{logging_dir}/deploy_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class TerraformDeployer:
    ...
    def generate_tfvars(self) -> bool:
        ...
        logger.info(f"Generating {self.tfvars_file}")
        ...
        try:
            ...
            logger.info(f"Generated {self.tfvars_file}")
            return True
...
Examples:
  ...
  python deploy.py config.yaml --workspace prod --verbose
        """
    )

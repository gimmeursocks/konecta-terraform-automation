...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'{logging_dir}/destroy_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class TerraformDestroyer:
    ...
    def generate_backend_config(self) -> Path:
        ...
        logger.info("Generating backend configuration")
        ...
        state_bucket = self.config.get('terraform_state_bucket', default_bucket)
        state_prefix = self.config.get('terraform_state_prefix', default_prefix)
        
        backend_file = self.backends_dir / f"{self.config['project_id']}.backend.conf"
        ...
        logger.info(f"Backend config created: {backend_file}")
        logger.info(f"Bucket: {state_bucket}")
        logger.info(f"Prefix: {state_prefix}")
        ...
        if 'terraform_state_prefix' not in self.config:
            logger.info(f"Using default prefix: {state_prefix}")
        
        logger.info(f"State location: gs://{state_bucket}/{state_prefix}/default.tfstate")
    ...
    def select_workspace(self) -> bool:
        ...
        if not self.workspace:
            logger.info("Using current workspace")
            result = subprocess.run(
    ...
    def confirm_destruction(self) -> bool:
        ...
        # First confirmation
        logger.info("\n" + "-" * 70)
        response1 = input("Type 'destroy' to confirm you want to proceed: ").strip()
        ...
        if self.project_id:
            logger.info("\n" + "-" * 70)
            response2 = input(f"Type the project ID '{self.project_id}' to confirm: ").strip()
    ...
    def destroy(self) -> bool:
        ...
        if remaining_resources > 0:
            logger.warning(f"{remaining_resources} resources still exist in state")
...
Examples:
  ...
  # Destroy with config file
  python destroy.py config.yaml

  # Destroy specific resources
  python destroy.py config.yaml --target module.vpc --target module.gcs

  # Destroy with workspace
  python destroy.py config.yaml --workspace dev

  # Auto-approve (dangerous!)
  python destroy.py config.yaml --auto-approve
...
Safety Features:
  ...
  - Detailed logging
    ...
    try:
        ...
        logger.warning("Infrastructure may be in inconsistent state")
        logger.warning("Run 'terraform state list' to check")
        sys.exit(130)
    except Exception as e:
